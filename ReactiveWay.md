...
Давайте взглянем как тоже самое можно сделать с помощью RxJS и его тип данных Observable:
...
Обратите внимание, как код легко читается, даже если вы не знакомы с ним. Кроме того, нет необходимости создавать внешние переменные для сохранения состояния, что делает код самодостаточным и затрудняет внесение ошибок. Вам также не нужно убирать за собой 💩, в виде снятия обработчика с события, и поэтому нет никакой речи об утечке памяти.
...
// любой объект имещий метод update будет работать
const subscriber1 = {
  update(message) {
...
Используя Observables, мы можем объявить, как реагировать на последовательность значений, которые они транслируют, вместо того, чтобы реагировать на отдельные значения. Мы можем эффективно копировать, преобразовывать и запрашивать последовательность, и эти операции будут применяться ко всем значениям последовательности.
